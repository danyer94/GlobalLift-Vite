import type { Language, LanguageToggleCopy } from '../content/siteContent';

type LanguageToggleProps = {
  value: Language;
  onChange: (value: Language) => void;
  copy: LanguageToggleCopy;
};
...
export function LanguageToggle({ value, onChange, copy }: LanguageToggleProps) {
  const indicatorClass = value === 'en' ? 'translate-x-full' : 'translate-x-0';

  return (
    <div
      className="relative grid grid-cols-2 items-center rounded-full border border-slate/70 bg-white/80 p-1 text-xs font-semibold shadow-soft backdrop-blur"
      role="radiogroup"
    ...
    >
      ...
      {options.map((option) => {
        ...
        const label = option.value === 'es' ? copy.spanish : copy.english;
        ...
        return (
          ...
          >
            ...
            <span className="hidden lg:inline text-[0.7rem]">{label}</span>
          </button>
        );
      })}
    </div>
