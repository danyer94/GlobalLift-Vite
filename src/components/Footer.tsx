        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr,1fr]">
          <div className="space-y-4">
            <a href="#top" className="flex items-center gap-3">
              ...
              <span className="text-sm font-semibold tracking-[0.2em] text-mist">Global Lift SRL</span>
            </a>
            <p className="text-sm text-muted max-w-md">{note}</p>
          </div>
          <div className="grid gap-3 text-sm text-muted sm:grid-cols-2">
            {items.map((item) => (
              <a key={item.label} href={item.href} className="transition-colors hover:text-mist">
                {item.label}
              </a>
            ))}
